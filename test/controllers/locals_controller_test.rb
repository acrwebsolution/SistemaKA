...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { codlocal: @local.codlocal, direccion: @local.direccion, distrito: @local.distrito }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { codlocal: @local.codlocal, direccion: @local.direccion, distrito: @local.distrito }
    assert_redirected_to local_path(assigns(:local))
  end
